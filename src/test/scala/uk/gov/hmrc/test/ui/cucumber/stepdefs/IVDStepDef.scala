...
class IVDStepDef extends ShutdownStepDef {
  ...
  And("""^the user enters (.*) into the (.*) input field$""") { (value: String, field: String) =>
    field match {
      ...
      case "Original amount" => findById("original").sendKeys(value) //ID can be subject to change (depending on code)
      case "Amended amount" => findById("amended").sendKeys(value) //ID can be subject to change (depending on code)
